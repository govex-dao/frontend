 ...
 */
export function ExplorerLink({ id, type, className = "" }: ExplorerLinkProps) {
    ...
    switch (type) {
        ...
        case "package":
            ...
            break;
        case "coin":
            pathSegment = `coin/${normalizeStructTag(id)}`;
            break;
        case "validator":
        ...
        default:
            ...
            console.warn(`ExplorerLink: Unknown type "${type}" provided for ID "${id}". Defaulting to object path.`);
    ...
    const copy = () => {
        navigator.clipboard
            ...
            .then(() => {
                ...
                toast.success("Copied to clipboard!", { id: "clipboard-copy" });
            })
            .catch(() => {
    ...
    return (
        <span className={`flex items-center gap-2 hover:bg-white/5 rounded-md p-1 text-text-light -m-1 ${className}`}>
            {copied ? (
