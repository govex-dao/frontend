...
function vestingProgress(vesting: VestingDisplayInfo): {
    ...
    const elapsedIterations = Math.min(Math.floor(elapsed / vesting.iterationPeriodMs), vesting.iterationsTotal);
    ...
    return (
        <div className="bg-card-elevated border border-border-subtle rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                ...
                <div className="flex items-center gap-1.5 text-text-muted">
                    {vesting.isCancellable ? <ShieldOff className="w-3 h-3" /> : <Shield className="w-3 h-3" />}
