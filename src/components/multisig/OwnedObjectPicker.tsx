...
export function OwnedObjectPicker({ accountId, value, onChange, label = "Object", typeFilter }: Props) {
    ...
    return (
        <>
            <Input label={label} value={value} onChange={(v) => onChange(v, "", false)} placeholder="0x... object ID" />
            {isLoading && <p className="text-[11px] text-text-muted">Loading account objects...</p>}
