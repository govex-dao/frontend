    ...
    return (
        <div className="space-y-3 border-t border-border-subtle pt-3">
            <div className="text-xs text-text-muted">
                ...
                move{selectedMoveObjectCount === 1 ? "" : "s"}, {selectedCapLockCount} cap lock
                {selectedCapLockCount === 1 ? "" : "s"} selected
            </div>
            {migrationBlockingError && (
            ...
            {selectedCapLockCount > 0 && (
                <div className="rounded-lg border border-yellow-400/20 bg-yellow-400/10 p-2 text-xs text-yellow-100">
                    Selected caps move into multisig custody now. The generated cap-lock intents still need normal
                    multisig approval and execution.
                </div>
            )}
