...
async function fetchMySpendingLimits(
  ...
  return spendingLimits.map(({ streamId, ...spendingLimit }) => ({
    ...spendingLimit,
...
function createStreamsTableResolver(client: SuiClient) {
  ...
  const vaultKeyFn = (accountId: string, vaultName: string): VaultKey =>
    `${accountId}::${vaultName}`;
  ...
  async function getStreamsTableId(accountId: string, vaultName: string): Promise<string | null> {
    ...
    try {
      ...
      const streamsTableId =
        get(fields, 'streams.fields.id.id') || get(fields, 'streams.fields.id');
