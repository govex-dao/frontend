 ...
 */
export function useOnChainCoinMetadata(
  coinTypes: string[],
  backendCoins?: CoinMetadata[],
) {
  const client = useSuiClient();

  const missing = coinTypes.filter(
    (ct) => ct && !backendCoins?.some((c) => c.coin_type === ct),
  );

  return useQuery({
    queryKey: ["onchain-coin-metadata", ...missing.sort()],
    queryFn: async (): Promise<CoinMetadata[]> => {
      const results = await Promise.all(
        missing.map(async (coinType): Promise<CoinMetadata | null> => {
          try {
